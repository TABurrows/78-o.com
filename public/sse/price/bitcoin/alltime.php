...
class GetDBFileLatest {
    ...
    // DB Query variables
    private $sel = "SELECT ask, bid, timestamp FROM GBP ORDER BY KEY DESC LIMIT 1500";
        ...
        try {
            ...
            if($result) {
                ...
                while($row = $result->fetchArray($this->DBFetchType)) {
                    $DBResults[$i] = $row;
                    if($row["ask"]) array_push($askResults, $row["ask"]);
                    if($row["bid"]) array_push($bidResults, $row["bid"]);
                    //if($row[""])
                    $i++;
                }
                $DBResults = array_reverse($DBResults);
                //Now get the max and min prices
                $rangePrices = ["max"=>["ask"=>max($askResults), "bid"=>max($bidResults)], "min"=>["ask"=>min($askResults), "bid"=>min($bidResults)]];
                //Now get the start and end times
                $rangeTimes = ["start"=>$DBResults[0]["timestamp"], "end"=>$DBResults[sizeof($DBResults)-1]["timestamp"] ];
                $DBData['results'] = $DBResults;
                $DBData['range'] = ["prices"=>$rangePrices,"times"=>$rangeTimes];
